...
private:
	...
	NcVar *posVar, *radVar, *BoxMatrixVar, *BoxStringVar, *PotStringVar, *StressVar;
...
public:
	...
	void WriteState(STATE const &c,Eigen::Matrix<double,Dim,Dim> &stress, int rec=-1);
	void ReadState(STATE &c,Eigen::Matrix<double,Dim,Dim> &stress, int rec);
...
void CStaticDatabase<Dim>::SetDimVar()
{
	...
	StressVar = File.add_var("Stress",ncDouble,recDim, dimDim,dimDim);
...
template <int Dim>
void CStaticDatabase<Dim>::WriteState(STATE const &s,Eigen::Matrix<double,Dim,Dim> &stress, int rec)
{
	...
	for(int d = 0 ; d < Dim ; d++)
		...
		{
			StressVar->set_cur(rec,d,d2);
			StressVar->put(&stress(d,d2),1,1,1);
		}
...
template <int Dim>
void CStaticDatabase<Dim>::ReadState(STATE &s,Eigen::Matrix<double,Dim,Dim> &stress, int rec)
{
	assert(Mode==NcFile::ReadOnly||Mode==NcFile::Write);
	assert(s.GetParticleNumber() == NP);
	GetDimVar();
	...
	StressVar->get(stress.data(),1,Dim,Dim);
	//Set the box
